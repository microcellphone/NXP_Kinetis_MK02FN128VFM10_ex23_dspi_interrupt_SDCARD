...
#define 	CS_LOW()	GPIO_PinWrite(BOARD_INITPINS_SPI_CS0_GPIO, BOARD_INITPINS_SPI_CS0_GPIO_PIN_MASK, false);	/* Set CS low */
#define	CS_HIGH()	GPIO_PinWrite(BOARD_INITPINS_SPI_CS0_GPIO, BOARD_INITPINS_SPI_CS0_GPIO_PIN_MASK, true);		/* Set CS high */
#define	IS_CS_LOW	!(GPIO_PinRead(BOARD_INITPINS_SPI_CS0_GPIO, BOARD_INITPINS_SPI_CS0_GPIO_PIN_MASK))		/* Test if CS is low */
#define	FORWARD(d)	UART0_PutByte(d)		/* Data forwarding function (console-out in this example) */
...
static void delay_us(uint32_t us)
{
    ...
    uint32_t ticks = us * (CLOCK_GetFreq(kCLOCK_CoreSysClk) / 1000000U);
    FTM_StartTimer(FTM0, kFTM_SystemClock);
    while (FTM_GetCurrentTimerCount(FTM0) < ticks)
        ;
    FTM_StopTimer(FTM0);
}
